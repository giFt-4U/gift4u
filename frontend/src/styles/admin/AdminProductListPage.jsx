...
const SortUp = styled.span`
    color: ${({ $active }) => ($active ? '#ff8c00' : '#c4c8cf')};
`;

const SortDown = styled.span`
    color: ${({ $active }) => ($active ? '#ff8c00' : '#c4c8cf')};
`;
...
const AdminProductListPage = () => {
    ...
    const sorted = [...filtered].sort((a, b) => {
        ...
        if (sortKey === 'category') {
            const aCategory = a.categoryName || getCategoryName(a.categoryId);
            const bCategory = b.categoryName || getCategoryName(b.categoryId);

            compareResult = aCategory.localeCompare(
                bCategory,
                'ko',
                { sensitivity: 'base' }
            );
        ...
        if (sortKey === 'price') {
            compareResult = Number(a.price || 0) - Number(b.price || 0);
        }

        if (sortKey === 'stock') {
            compareResult = Number(a.stock || 0) - Number(b.stock || 0);
        }

        if (sortKey === 'status') {
            // 오름차순: 판매중 우선 / 내림차순: 비활성 우선
            const aStatus = getProductActive(a) ? 0 : 1;
            const bStatus = getProductActive(b) ? 0 : 1;

            compareResult = aStatus - bStatus;
        }

        if (compareResult === 0) {
            compareResult = Number(b.id || 0) - Number(a.id || 0);
        }
